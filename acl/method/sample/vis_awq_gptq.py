...
gptq_front_13b_ppl = [11.42157841, 9.97035408, 9.068058014, 7.738716125, 6.992593288, 6.514576435, 5.939143181, 5.798785686, 5.450032711, 5.361387253, 5.309822559, 5.279051304, 5.212362766, 5.162135601, 5.129513741, 5.061660767, 5.044354439, 4.984623909]


all_rho, _ = stats.spearmanr(hqq_13b_ppl, awq_13b_ppl)
all_tau, _ = stats.kendalltau(hqq_13b_ppl, awq_13b_ppl)

print(f'all_rho : {all_rho}, all_tau : {all_tau}')


all_rho, _ = stats.spearmanr(hqq_front_13b_ppl, awq_front_13b_ppl)
all_tau, _ = stats.kendalltau(hqq_front_13b_ppl, awq_front_13b_ppl)
print(f'all_rho : {all_rho}, all_tau : {all_tau}')
print(f'hqq_13b_ppl : {len(hqq_13b_ppl)}')
print(f'awq_13b_ppl : {len(awq_13b_ppl)}')

print(f'hqq_front_13b_ppl : {len(hqq_front_13b_ppl)}')
print(f'awq_front_13b_ppl : {len(awq_front_13b_ppl)}')
print(f'gptq_front_13b_ppl : {len(gptq_front_13b_ppl)}')
print(f'font_size : {font_size}')
# plt.scatter(hqq_front_13b_ppl, awq_front_13b_ppl, c=front_13b_bits, s=5)
# plt.scatter(hqq_front_13b_ppl, gptq_front_13b_ppl, c=front_13b_bits, s=5, label='13b')


# fig, axes = plt.subplots(ncols=1, nrows=1, figsize=(5, 4))


# im = axes.scatter(hqq_13b_ppl, awq_13b_ppl, c=awq_13b_bits, s=5)
# im = axes[0].scatter(hqq_front_13b_ppl, awq_front_13b_ppl, c=front_13b_bits, s=5)
# axes.set_title("Wikitext2 Perplexity")
# axes.set_xlim([4, 20])

# axes.set_xticks(np.arange(5, 21, 5))
# axes.set_yticks(np.arange(5, 8, 1))
# axes.set_xscale('log', base=10)

# axes.text(5, 7, "Rho: 0.86\nTau:  0.7", bbox=dict(facecolor='none', edgecolor='black'))

# axes.table(
#     cellText=[0.86, 0.7],  # table data
#     colLabels=['Rho', 'Tau'],  # column label (column name)
#     colWidths=[0.1],  # column width
#     # colColours=list_col_color,  # column label color
#     loc='upper right'  # table location
# )
# axes.colorbar()
fig, axes = plt.subplots(ncols=2, nrows=1, figsize=(8, 4))
im = axes[0].scatter(hqq_front_13b_ppl, awq_front_13b_ppl, c=front_13b_bits, s=5)
# axes[0].set_title('Pareto-frontier', fontdict=font)
# axes[0].set_ylabel('PPL', fontdict=font)
# axes[0].set_xlabel('PPL', fontdict=font)
im = axes[1].scatter(hqq_13b_ppl, awq_13b_ppl, c=awq_13b_bits, s=5)
# axes[1].set_title('Random Sampling', fontdict=font)
# axes[1].set_xlabel('PPL', fontdict=font)

fig.tight_layout()  
fig.subplots_adjust(right=1.0)
fig.colorbar(im, ax=axes)
# cbar_ax = fig.add_axes([0.85, 0.4, 0.01, 0.2])
# fig.colorbar(awq_13b_bits, cax=cbar_ax, label='Bits')
...
# plt.xlabel('HQQ')
# plt.ylabel('AWQ')
fig_path = '/NAS/SJ/nsgaquant/acl/method/sample/compare_hqq_awq'
# fig_path = '/NAS/SJ/nsgaquant/acl/method/sample/random_sample'
