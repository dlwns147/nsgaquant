...
from search_space.llama import LlamaBlockSearchSpace
...
class Search:
    def __init__(self, config, accelerator, device_map, kwargs):
        ...
        self.search_space = LlamaBlockSearchSpace(
...
class AuxiliarySingleLevelProblem(Problem):
    ...
    def __init__(self, search_space, predictor, config, method, latency_table):
        ...
        super().__init__(n_var=n_block, n_obj=2, n_constr=4, type_var=int)

        self.ss = search_space
        self.predictor = predictor
        self.xl = np.zeros((n_block))
        self.xu = np.ones((n_block))
