...
@torch.no_grad()
def eval_loss(model, testenc, bs=1, seqlen=2048, loss_func='cross_entropy', dense_logits_list=None, device=None):
    # Get input IDs
    testenc = testenc.input_ids
    ...
    loss_sum = torch.stack(losses).sum() / (seqlen * n_sample)

    return loss_sum.item()


def eval_metric(model, accelerator, metric, loader, seqlen, loss_func='cross_entropy', dense_logits_list=None, device=None):
    # accelerator.wait_for_everyone()
    if metric == 'ppl':
        return eval_ppl(model, accelerator, loader, bs=1, seqlen=seqlen, device=None)
    elif metric == 'loss':
        return eval_loss(model, accelerator, loader, bs=1, seqlen=seqlen, loss_func=loss_func, dense_logits_list=dense_logits_list, device=device)
    else:
