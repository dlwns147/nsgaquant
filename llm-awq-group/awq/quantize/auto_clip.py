...
def auto_clip_layer(
    ...
    for i_b in range(w.shape[0] // oc_batch_size):
        ...
        org_min_val = w.abs().amin(dim=-1, keepdim=True)  # co, 1, n_group, 1
...
@torch.no_grad()
def apply_clip(module, clip_lists, arch):
    ...
    for linear, linear_bits_list in arch['linear'].items():
        for blk_idx, linear_bits in enumerate(linear_bits_list):
            linear_name = f'model.layers.{blk_idx}.{linear}'
            for name, max_val, min_val in clip_lists[linear_bits]:
                if linear_name in name:
                    # print(f'linear_name : {linear_name}, name : {name}, max_val : {max_val.shape}')
