...
def apply_scale(module, scales_list, arch, input_feat_dict=None):

    for linear, linear_bits_list in arch['linear'].items():

        if linear in ['self_attn.k_proj', 'self_attn.v_proj', 'mlp.gate_proj']:
            continue

        for blk_idx, linear_bits in enumerate(linear_bits_list):
            layer_name = f'model.layers.{blk_idx}.{linear}'

            for prev_op_name, layer_names, scales in scales_list[linear_bits]:
                if layer_name in layer_names:
                    
                    # print(f'layer_name : {layer_name}, prev_op_name : {prev_op_name}, layer_names : {layer_names}, linear_bits : {linear_bits}, scales : {scales.shape}')
