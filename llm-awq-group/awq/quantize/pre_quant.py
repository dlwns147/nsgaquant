...
def apply_awq(model, awq_results, arch):
    apply_scale(model, {b: v["scale"] for b, v in awq_results.items()}, arch) 
    apply_clip(model, {b: v["clip"] for b, v in awq_results.items()}, arch)
