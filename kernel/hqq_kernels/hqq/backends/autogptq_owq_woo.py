...
class GPTQLinear_owq_woo(nn.Module):
    QUANT_TYPE = "cuda-old"
    ...
    def __init__(
        ...
        self.register_buffer(
            'oweight', torch.zeros((outlierfeatures, self.outfeatures), dtype=torch.float)
        )
        self.register_buffer(
...
def patch_hqq_to_gptq_owq(layer, patch_params, load = False):
    hqq_layer = None
    ...
    gptq_layer = GPTQLinear_owq_woo(nbits, 
                            group_size, 
                            infeatures, 
                            outfeatures, 
                            bias).to(device)
    ...
    if not load:
        ...
        if 'o_proj' not in gptq_layer.name:
            ...
            for idx in hqq_layer.outlieridx:
                W_deq[:, idx] = torch.zeros(W_deq.shape[0], dtype=W_deq.dtype, device=W_deq.device)
...
def patch_hqq_to_gptq_owq_load(layer, patch_params):
    return patch_hqq_to_gptq_owq(layer, patch_params, load = True)