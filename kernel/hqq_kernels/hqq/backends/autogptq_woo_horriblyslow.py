...
class GPTQLinear_woo(nn.Module):
    ...
    def pack(self, W, scales, zeros, g_idx=None):
        ...
        self.scales = scales.clone().to(dtype=W.dtype)
        # if linear.bias is not None:
        #     self.bias = linear.bias.clone().to(dtype=W.dtype)
        ...
        self.qzeros = torch.from_numpy(qzeros).to(scales.device)

    def forward(self, x: torch.Tensor):
        # import code; code.interact('forward test', local=dict(globals(), **locals()))
        ...
        # import code; code.interact('GPTQLinear woo', local=dict(globals(), **locals()))
        return out
