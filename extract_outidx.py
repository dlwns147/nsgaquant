    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in meta['sequential'][3]: # down
                ...
                outlier[f"{meta['prefix']}.{i}.{name}"] = gptq_owq[name].quantizer.out_ids.tolist()
                gptq_owq[name].free()
                # import pdb; pdb.set_trace()
                print(f'name : {name}, out_ids : {gptq_owq[name].quantizer.out_ids.tolist()}, W : {W.shape}, W[out_ids] : {W[:, gptq_owq[name].quantizer.out_ids.tolist()].shape}')
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in meta['sequential'][0] + meta['sequential'][2]:
                W = subset[name].weight.data
                # outlier[f"{meta['prefix']}.{i}.{name}"] = [out_ids, W[out_ids]]
                outlier[f"{meta['prefix']}.{i}.{name}"] = out_ids
                print(f'name : {name}, out_ids : {out_ids}, W : {W.shape}, W[out_ids] : {W[:, out_ids].shape}')
