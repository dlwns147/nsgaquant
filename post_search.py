...
def main(args):
    ...
    if len(args.target_bits_range) == 2:
        assert args.target_bits_range[0] >= args.small_model_bits and args.target_bits_range[1] <= args.large_model_bits, f'Target bits range should be in [small model bits, large model bits]'
    ...
    for idx in tqdm(I):
        print(f'Selected arch[{idx}] bits: {pf[idx, 1]:.4f}, metric: {pf[idx, 0]:.4f}')
    ...
    for idx in tqdm(I):
        arch = ps[idx]
        metric, complexity = evaluator.eval(arch, 'ppl')
    ...
    with open(os.path.join(args.save, args.results_csv_file), 'w') as f:
        writer = csv.writer(f)
        writer.writerow([args.sec_obj, 'ppl', 'metric'])
        for c, p, m in zip(complexity_list, ppl_list, metric_list):
            writer.writerow([c, p, m])
